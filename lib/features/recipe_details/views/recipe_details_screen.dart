...
class RecipeDetailScreen extends GetView<RecipeController> {
  ...
  _IngredientParsed _parseIngredient(String raw) {
    ...
    if (raw.toLowerCase().contains('to taste')) {
      String nm = raw
          .replaceAll(
            RegExp(r'\s*to\s*taste\s*', caseSensitive: false),
            '',
          )
          .trim();

      if (nm.isNotEmpty) {
        nm = nm[0].toUpperCase() + nm.substring(1);
      } else {
        nm = raw;
      }

      return _IngredientParsed(
        amount: 'To taste',
        name: nm,
      );
    }

    // Pattern 1: Starts with quantity / units
    ...
    // Pattern 2: Ends with quantity in parentheses
    final RegExp suffixParenthesesRegex =
        RegExp(r'^(.*?)\s*\(([^)]+)\)$');

    final matchParentheses =
        suffixParenthesesRegex.firstMatch(raw);
  ...
  Recipe _mapToRecipe(dynamic args) {
    try {
      ...
      // Try controller recipe first
      try {
        dynamic ctrlRecipe;
        final dynamic dynController = controller;

        if (dynController.recipe != null &&
            dynController.recipe.value != null) {
      ...
      if (args is Recipe) {
        ...
        return Recipe(
          ...
          imageUrl: r.imageUrl,
          prepTime: r.prepTime,
          ingredients: r.ingredients.isNotEmpty
              ? r.ingredients
              : _getDefaultIngredients(r.name),
          steps: r.steps.isNotEmpty
              ? r.steps
              : _getDefaultSteps(r.name),
      ...
      if (args is String || args is int) {
        ...
        final lookupRecipe =
            _lookupRecipeInHomeController(targetId);
        ...
        rating = double.tryParse(
              (args['rating'] ?? 4.7).toString(),
            ) ??
            4.7;

        reviews = int.tryParse(
              (args['reviews'] ?? 45).toString(),
            ) ??
            45;

        difficulty =
            (args['difficulty'] ?? 'Medium').toString();

        imageUrl =
            (args['imageUrl'] ?? args['image'] ?? '').toString();
        ...
        ingredients =
            List<String>.from(args['ingredients'] ?? []);

        steps = List<String>.from(
          args['steps'] ??
              args['instructions_list'] ??
              [],
        );

        instructions =
            (args['instructions'] ?? '').toString();

        youtubeUrl =
            (args['youtubeUrl'] ?? '').toString();
        ...
        name = _safeGetProperty(
          obj,
          ['name', 'title'],
          'Recipe Details',
        ).toString();

        cuisine = _safeGetProperty(
          obj,
          ['cuisine'],
          '',
        ).toString();

        category = _safeGetProperty(
          obj,
          ['category'],
          '',
        ).toString();
        ...
        difficulty = _safeGetProperty(
          obj,
          ['difficulty'],
          'Medium',
        ).toString();

        imageUrl = _safeGetProperty(
          obj,
          ['image', 'imageUrl', 'recipeImage'],
          '',
        ).toString();
        ...
        instructions = _safeGetProperty(
          obj,
          ['instructions'],
          '',
        ).toString();

        youtubeUrl = _safeGetProperty(
          obj,
          ['youtubeUrl'],
          '',
        ).toString();
      ...
      return Recipe(
        ...
        prepTime: 25,
        ingredients:
            _getDefaultIngredients(
          'Recipe Error Details',
        ),
        steps:
            _getDefaultSteps(
          'Recipe Error Details',
  ...
  Recipe _convertToRecipe(dynamic foundModel) {
    ...
    final String name = _safeGetProperty(
      foundModel,
      ['name', 'title'],
      'Recipe Details',
    ).toString();

    List<String> ingredients = parseList(
      _safeGetProperty(
        foundModel,
        ['ingredients'],
        null,
  ...
  static dynamic _safeGetProperty(
    dynamic obj,
    List<String> fields,
  ...
  static dynamic getFieldValueDirect(
    dynamic obj,
    String field,
  ) {
    try {
      ...
      if (field == 'instructions_list') {
        return obj.instructions_list;
      }
      if (field == 'instructions') {
        return obj.instructions;
      }
      if (field == 'youtubeUrl') {
        return obj.youtubeUrl;
      }
      if (field == 'isFavorite') {
  ...
  // ==========================================
  // HOME CONTROLLER LOOKUP
  // ==========================================

  Recipe? _lookupRecipeInHomeController(
    String targetId,
  ) {
    try {
      ...
      debugPrint(
        "Error in HomeController lookup: $e",
      );
    }
...
// ============================================================================
// WHATSAPP-STYLE COOKING AUDIO VOICE BAR
// ============================================================================
...
class WhatsAppVoiceBar extends StatefulWidget {
  ...
  @override
  State<WhatsAppVoiceBar> createState() =>
      _WhatsAppVoiceBarState();
}

class _WhatsAppVoiceBarState
    extends State<WhatsAppVoiceBar> {
    ...
    _secondTimer = Timer.periodic(
      ...
      (timer) {
        ...
        try {
          ...
          final bool ctrlSpeaking =
              dynController
                  .isSpeaking
                  .value;

          final bool ctrlPaused =
              dynController
                  .isPaused
                  .value;
          ...
          if (ctrlSpeaking &&
            ...
            setState(() {
              ...
              final int stepStartSec =
                  ctrlStep *
                      _secondsPerStep;

              final int stepEndSec =
                  (ctrlStep + 1) *
                          _secondsPerStep -
                      1;
  ...
  Future<void> _play() async {
    if (_isTtsOperationInProgress) {
      return;
    }

    final int totalSteps =
        widget.cleanSteps.length;
  ...
  void _resume() {
    ...
    try {
      ...
      dynController.speakSpecificStep(
        steps,
        _localCurrentStep,
      );
